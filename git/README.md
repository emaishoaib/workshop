...
| `grbe edit` | Fuzzy-pick (multi-select with Tab) commit(s) from those on the current branch vs the default branch, and surface them in VS Code to edit one at a time, oldest first |
| `grbe done` | Finish editing the current commit in a `grbe edit` session — if you changed anything, commits it back in (reusing the original commit's message) and continues the rebase; if you didn't, discards it. If more commits were selected, the next one is then surfaced in VS Code the same way; once the last one is done, the stash (if any) is restored. If continuing the rebase hits a genuine conflict, resolve it, stage it, and run `grbe done` again rather than `git rebase --continue` directly |
| `grbe onto` | Fuzzy-pick a local branch to rebase onto, then fuzzy-pick the fork point SHA from commits on the current branch |
| `grbe all` | Interactive rebase over every commit on the current branch vs the default branch — no guessing a commit count. On the default branch itself, fuzzy-pick the oldest commit to include instead, and it rebases that commit and everything after it |
...
- **Re-create this repo on GitHub** — run from inside a local repo: deletes it on GitHub, re-creates it with the same visibility and description — under the same name, or a new one you give it — and pushes every local branch and tag. With a new name, it repoints `origin` and offers to rename the local folder to match. Use it after rewriting history, to purge the old commits GitHub would otherwise keep reachable by ID. Issues, PRs, releases, stars and settings are lost — it shows their counts, warns about branches that exist only on GitHub, and makes you type the repo name to confirm. It needs `gh`'s `delete_repo` permission, which it never grants itself — without it, it stops and prints the command to grant it (`gh auth refresh -h github.com -s delete_repo`); afterwards it reminds you to remove it again (`gh auth refresh -h github.com -r delete_repo`)
