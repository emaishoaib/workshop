...
/// calls into a domain-specific owner (Menubar, TrackpadEvents, SparkleDelegate,
...
class PreferencesEvents {
    ...
    static func initialize() {
        ...
        initialized = true
        ShortcutRegistry.initializePreferencesDependentState()
        // (removed: TrackpadEvents.toggle(...) — trackpad-gesture switching was deleted)
        // (removed: LoginItem.applyCurrentPreference() — the in-app `startAtLogin` launchd-plist
        // toggle was deleted along with LoginItem.swift; autostart is handled by the user's own
        // LaunchAgent instead)
    }
    ...
    static func preferenceChanged(_ key: String) {
        ...
        // `LicenseManager.computeState()` is hardcoded to `.pro` (see pro/license/LicenseManager.swift),
        // so `isProLocked` is always false and this branch (navigate to the Settings upgrade tab)
        // can never fire. Dropped as part of extracting the engine from settings-window/.
        ShortcutRegistry.preferenceChanged(key)
        switch key {
