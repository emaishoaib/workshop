...
class Preferences {
    ...
    /// Hardcoded rather than read from UserDefaults — there's no Settings UI to customize this
    /// list anymore, so it's no longer a real per-user preference. This is exactly the built-in
    /// default list AltTab shipped with (see the deleted `defaultExceptions()`); kept as-is since
    /// it does real, non-optional work: hides Finder when it has no open windows, and disables
    /// shortcuts in fullscreen remote-desktop/VM apps.
    static let exceptions: [ExceptionEntry] = [
    ...
    /// Remove an override (the user "unlinks" it from the global). Used to also clear a remembered
    /// Pro index in `ProTransitionState` for the 3 Pro-gated overrides on shortcut 0 — that whole
    /// snapshot/restore mechanism is gone along with `pro/`, so this is now a plain removal.
