...
class ATShortcut {
    ...
    private func modifiersMatch(_ modifiers: CarbonModifierFlags) -> Bool {
        ...
        let holdModifiersCleaned = ShortcutRegistry.shortcuts[Preferences.indexToName("holdShortcut", session?.shortcutIndex ?? 0)]?.shortcut.carbonModifierFlags.cleaned() ?? 0
    ...
    func redundantSafetyMeasures() {
        ...
        if let session = SwitcherSession.current, !session.forceDoNothingOnRelease, Preferences.effectiveShortcutStyle(session.shortcutIndex) == .focusOnRelease {
            if let currentHoldShortcut = ShortcutRegistry.shortcuts[Preferences.indexToName("holdShortcut", session.shortcutIndex)],
