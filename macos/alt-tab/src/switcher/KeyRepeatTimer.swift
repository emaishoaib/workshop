...
class KeyRepeatTimer {
    ...
    static func startRepeatingKeyPreviousWindow() {
        if let shortcut = ShortcutRegistry.shortcuts["previousWindowShortcut"],
    ...
    static func startRepeatingKeyNextWindow() {
        ...
        if let shortcut = ShortcutRegistry.shortcuts[nextWindowShortcutName] {
            startTimerForRepeatingKey(shortcut) {
    ...
    private static func holdModifierIsReleased() -> Bool {
        guard let session = SwitcherSession.current,
              let holdShortcut = ShortcutRegistry.shortcuts[Preferences.indexToName("holdShortcut", session.shortcutIndex)] else {
            return true
        }
