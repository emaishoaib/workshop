...
class TilesPanel: NSPanel {
    ...
    private func repositionOrFreeze() {
        // Used to keep the panel's top edge frozen in place while search-in-switcher grew/shrank
        // the panel height as results changed. Search-in-switcher was deleted, so this always
        // takes the "reposition normally" branch now.
...
extension TilesPanel: NSWindowDelegate {
    ...
    func windowDidBecomeKey(_ notification: Notification) {
        ...
        DispatchQueue.main.async {
            ...
            // (removed: `if TilesView.isSearchEditing { MainMenu.toggleEditMenu(true) }` — search-in-switcher was deleted)
        }
