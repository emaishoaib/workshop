...
class TilesView {
    ...
    /// Search-in-switcher (the "S"/"Space" shortcuts, the `.searchOnRelease` shortcut style, the
    /// floating `NSSearchField`, and this whole state machine) was deleted — it's structurally
    /// unreachable now (no shortcut is ever bound to enter it). `hasMarkedText()` is kept as a
    /// stub since `CursorEvents`/`TileOverView` still call it as an IME-composing guard.
    static func hasMarkedText() -> Bool { false }
