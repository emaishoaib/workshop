...
class Window {
    ...
    func refreshThumbnail(_ screenshot: CALayerContents) {
        ...
        // `position` used to be needed here to reposition `PreviewPanel` (deleted); only `size` is used now.
        if let size,
           let view = (TilesView.recycledViews.first { $0.window_?.cgWindowId == cgWindowId }) {
            ...
            // (removed: PreviewPanel.updateIfShowing(...) — PreviewPanel.swift was deleted)
        }
    }
