...
class Windows {
    ...
    // Always `false` now — search-in-switcher was deleted, so nothing ever sets these true
    // anymore. Kept (rather than threading a removal through `SelectionResolver`'s kernel
    // struct) since `SelectionInputs` still has `restoreDefaultOnSearchClear`/`bestMatchOnSearchChange` fields.
    ...
    static func shouldDisplay(_ window: Window) -> Bool {
        // Used to also check `Search.matches(window, query: ...)` — search-in-switcher was
        // deleted (the query is now always empty, which `Search.matches` always passed anyway).
    ...
    static func voiceOverWindow(_ windowIndex: Int = (SwitcherSession.current?.selectedIndex ?? 0)) {
        ...
        // (removed: `if TilesView.isSearchEditing { return }` guards — search-in-switcher was deleted)
    ...
    private static func sort() {
        ...
        // `searchActive` is always false now — search-in-switcher was deleted, so `searchQuery`
        // never becomes non-empty. Kept as an explicit local (rather than removing the parameter
        // from `WindowOrderResolver.isOrderedBefore`) since that's a shared pure kernel.
        ...
        // Precompute each window's ordering facts once, then sort on the snapshots.
    ...
    static func removeWindows(_ windows: [Window], _ addWindowlessWindowIfNeeded: Bool) {
        ...
        // (removed: PreviewPanel.clearIfShowing loop — PreviewPanel.swift was deleted)
        for w in windows {
